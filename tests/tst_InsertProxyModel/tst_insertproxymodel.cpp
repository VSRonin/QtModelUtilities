            ...
            if (tempModel) {
                for (auto &&addViaProxy :
                    ...
                    for (auto &&indexToInsert :
                         {std::make_pair(0, QByteArrayLiteral("Begin")), std::make_pair(tempModel->rowCount(), QByteArrayLiteral("End")),
...
void tst_InsertProxyModel::testCommitSubclass()
{
    ...
    {
    public:
        explicit InsertProxyModelCommit(QObject *parent = Q_NULLPTR)
