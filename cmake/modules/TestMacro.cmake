macro(BasicTest TestName)
    ...
    project(${testProjectName} LANGUAGES CXX)
    ...
    if(${Qt5Test_VERSION} VERSION_LESS "5.11.0")
        file(DOWNLOAD "https://code.qt.io/cgit/qt-creator/qt-creator.git/plain/src/shared/modeltest/modeltest.cpp" "${CMAKE_CURRENT_SOURCE_DIR}/modeltest.cpp" STATUS ModelTestDownloadStatus)
        list(GET ModelTestDownloadStatus 0 ModelTestDownloadStatusCode)
        if(ModelTestDownloadStatusCode EQUAL 0)
            file(DOWNLOAD "https://code.qt.io/cgit/qt-creator/qt-creator.git/plain/src/shared/modeltest/modeltest.h" "${CMAKE_CURRENT_SOURCE_DIR}/modeltest.h" STATUS ModelTestDownloadStatus)
            list(GET ModelTestDownloadStatus 0 ModelTestDownloadStatusCode)
            if(ModelTestDownloadStatusCode EQUAL 0)
                target_sources(${targetName} PRIVATE modeltest.cpp modeltest.h)
            else()
    ...
    if(BUILD_STATIC_LIBS)
        target_compile_definitions(${targetName} PRIVATE MODELUTILITIES_STATIC)
    endif()
    target_include_directories(${targetName} PRIVATE ../../src)
    target_link_libraries(${targetName} PRIVATE Qt5::Core Qt5::Test modelutilities)
    if(NOT NO_GUI)
        target_link_libraries(${targetName} PRIVATE Qt5::Gui)
    endif()
    ...
    if(TEST_OUTPUT_XML)
        ...
        set(Test_Output_FileName "${Test_Output_FileName}_tstres.xml")
