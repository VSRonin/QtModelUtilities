...
private:
    ...
    void onInsertRowEnd(){
        bool okDialog=false;
        const int numRows = QInputDialog::getInt(this,QObject::tr("Number of Rows"),QObject::tr("Select the number of rows to insert"),1,1,std::numeric_limits<int>::max()/2,1,&okDialog);
        if(!okDialog)
            return;
    ...
    void onInsertColumnEnd(){
        bool okDialog=false;
        const int numCols = QInputDialog::getInt(this,QObject::tr("Number of Columns"),QObject::tr("Select the number of columns to insert"),1,1,std::numeric_limits<int>::max()/2,1,&okDialog);
        if(!okDialog)
            return;
